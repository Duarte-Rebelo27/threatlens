...
def analyze_logs(
    ...
    """
    all_alerts = run_detectors(logs, config=config)
...
def run_detectors(logs: List[Dict], config: Optional[Dict] = None) -> List[Dict]:
    config = config or get_config()

    alerts = []
    alerts.extend(detect_brute_force_attempts(logs, config))
    alerts.extend(detect_suspicious_paths(logs, config))
    alerts.extend(detect_abnormal_time(logs, config))
    return alerts
